...
def open_sales_history_window(): 
    """Mở cửa sổ xem lịch sử bán hàng."""
    history_window = Toplevel(root)
    history_window.title("Lịch sử bán hàng")
    center_window(history_window, 1200, 600) # Căn giữa cửa sổ
    ...
    Label(history_window, text="LỊCH SỬ BÁN HÀNG", 
          font=("Arial", 20, "bold"), 
          fg='#E64A19', 
          bg='#f5f5f5').pack(pady=20)
    
    # Placeholder Treeview cho lịch sử
    history_frame = Frame(history_window, padx=20, bg='#f5f5f5'); history_frame.pack(pady=10, fill='both', padx=30, expand=True)
    
    columns_history = ("Mã Đơn", "Ngày", "Khách hàng", "Tên SP", "Số lượng", "Tổng tiền (VND)")
    tree_history = ttk.Treeview(history_frame, columns=columns_history, show='headings', height=15)

    for col in columns_history: tree_history.heading(col, text=col, anchor=CENTER)

    tree_history.column("Mã Đơn", width=80, anchor=CENTER); 
    tree_history.column("Ngày", width=100, anchor=CENTER)
    tree_history.column("Khách hàng", width=150, anchor=W); 
    tree_history.column("Tên SP", width=250, anchor=W);
    tree_history.column("Số lượng", width=80, anchor=CENTER); 
    tree_history.column("Tổng tiền (VND)", width=150, anchor=E)

    scrollbar_history = ttk.Scrollbar(history_frame, orient="vertical", command=tree_history.yview)
    tree_history.configure(yscrollcommand=scrollbar_history.set)
    ...
    def load_history_data():
        ...
        try:
            for i in tree_history.get_children(): tree.delete(i)
            cur = conn.cursor()
            
            # CẬP NHẬT TRUY VẤN: Lấy chi tiết sản phẩm và nhóm lại để hiển thị từng dòng cho mỗi sản phẩm
            query = """
                SELECT 
                    hd.MaDon, 
                    hd.NgayTaoDon, 
                ...
                FROM 
                    hoadon hd
                JOIN 
                    chitiet_hoadon ct ON hd.MaDon = ct.MaDon
                ORDER BY
                    hd.NgayTaoDon DESC, hd.MaDon DESC
            """
            cur.execute(query)
            ...
            for row in rows:
                formatted_row = list(row)
                ma_don = formatted_row[0]
                total_payment = formatted_row[5]
                
                # Chỉ hiển thị Mã đơn, Ngày, Khách hàng và Tổng tiền ở dòng đầu tiên của đơn hàng
                if ma_don == last_ma_don:
                    # Dòng tiếp theo của cùng một đơn hàng
                    formatted_row[0] = "" # Xóa Mã Đơn
                    formatted_row[1] = "" # Xóa Ngày
                    formatted_row[2] = "" # Xóa Khách hàng
                    formatted_row[5] = "" # Xóa Tổng tiền
                else:
                    # Dòng đầu tiên của đơn hàng mới
                    formatted_row[5] = format_currency(total_payment) # Định dạng Tổng tiền
                
                tree_history.insert("", END, values=(
                    formatted_row[0], 
                    formatted_row[1], 
                    formatted_row[2], 
                    formatted_row[3], # Tên SP
                    formatted_row[4], # Số lượng
                    formatted_row[5] # Tổng tiền (chỉ hiển thị ở dòng đầu)
                ))
                last_ma_don = ma_don # Cập nhật Mã đơn hàng cuối cùng
            
            if not rows:
                Label(history_frame, 
                      text="Chưa có đơn hàng nào được thanh toán và lưu lịch sử.", 
                      fg='gray', 
                      bg='#f5f5f5', 
                      font=("Arial", 12)).pack(pady=20) 
                
        except mysql.connector.Error as e:
            if e.errno == 1146: # 1146 is ER_NO_SUCH_TABLE (hoadon or chitiet_hoadon)
                messagebox.showwarning("Thiếu CSDL", "Chưa tìm thấy các bảng 'hoadon' hoặc 'chitiet_hoadon'. Vui lòng tạo bảng trong MySQL.")
            else:
                messagebox.showerror("Lỗi CSDL", f"Lỗi truy vấn lịch sử: {e}")
        except Exception as e:
            messagebox.showerror("Lỗi", str(e))
        finally:
            if conn and conn.is_connected():
    ...
    Button(history_window, 
           ...
           fg='white',
           font=("Arial", 10, 'bold'),
           command=go_back_to_main).pack(pady=30)
...
# Giao diện chính
Label(root, text=" QUẢN LÝ CỬA HÀNG THIẾT BỊ ĐIỆN TỬ", fg="black", font=("Arial", 16, "bold")).pack(pady=30)

frame = Frame(root); frame.pack(pady=30)

# Nút 1: Giao diện Bán hàng
btn_sales = Button(frame, text="Giao diện Bán hàng", width=20, height=2, bg="#B3E5FC",
                   command=lambda: [root.withdraw(), open_sales_interface_window()])
btn_sales.pack(pady=10)

# Nút 2: Kho                                                        
btn_edit = Button(frame, text="Kho", width=20, height=2, bg="lightgreen",
                   command=lambda: [root.withdraw(), open_employee_management_window()])
btn_edit.pack(pady=10)

# Nút 3: Lịch sử bán hàng
btn_history = Button(frame, text="Lịch sử bán hàng", width=20, height=2, bg="#FFCC80",
                   command=lambda: [root.withdraw(), open_sales_history_window()])
btn_history.pack(pady=10)

# Nút 4: Thoát
btn_thoat = Button(frame, text="Exit", width=20, height=2, bg="lightcoral", command=root.destroy)
